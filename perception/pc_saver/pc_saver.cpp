...
void save_to_disk(const rs2::frame& frame, int i, const std::string& frame_name);
...
int main() {
	...
	//HUANG: 440 hit 1
		...
		// cfg.enable_stream(RS2_STREAM_INFRARED, RS2_FORMAT_Y8, 30);  //
		// cfg.enable_stream(RS2_STREAM_DEPTH, 640, 480, RS2_FORMAT_Z16, 30); // 
		...
		while (captureLoop == true) {
			...
			//	 单台相机
				...
				//转换点云数据的坐标与实际场景相对应,改变默认情况下，X轴和Y轴的坐标与实际相反的问题
				//for (int i = 0; i < cloud->points.size(); i++)
				//{
				//	cloud->points[i].x = -cloud->points[i].x;
				//	cloud->points[i].y = -cloud->points[i].y;
				//}
				if ((ch = kbhit())!=0) {//如果有按键按下，则_kbhit()函数返回真
					std::cout<<"Press Enter to save data."<<std::endl;
					...
					if (ch == 13) {
						...
						//also save depth image, color image and metadata
						std::stringstream frame_name;
						frame_name << "aligned_color_frame";
						save_to_disk(aligned_color_frame, i, frame_name.str());
						std::stringstream frame_name1;
						frame_name1 << "aligned_depth_frame";
						save_to_disk(aligned_depth_frame, i, frame_name1.str());
						std::stringstream frame_name2;
						frame_name2 << "before_depth_frame";
						save_to_disk(before_depth_frame, i, frame_name2.str());
...
void save_to_disk(const rs2::frame& frame, int i, const std::string& frame_name){
	if(auto vf = frame.as<rs2::video_frame>()){
		...
		png_file << frame_name +to_string(i) << ".png";
		...
		csv_file << frame_name + "_metadata_" + to_string(i) << ".csv";
